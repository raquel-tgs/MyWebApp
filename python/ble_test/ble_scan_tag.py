...
class boldtag:
    ...
    async def new(self, device, connect=False, max_retry=3,timeout=15):
        ...
        try:
            ...
            self.items.append(ble_tag(device=device, webapp=self.webapp, index=self._index,scanner_param=self.scanner_param,serv_uuid_Custom_Service=self.serv_uuid_Custom_Service,directory=self.directory))
    ...
    async def connect(self,index, max_retry=3,timeout=15):
        ...
        if tag is not None:
            return await tag.connect(max_retry=max_retry,timeout=15)
